...
<article class="large-12 columns prod-card">
	...
	<div id="primary_block" >
		...
		<!-- left infos -->
		<div class="large-7 columns prod-right" id="pb-left-column">
			
			{if $product->description_short OR $packItems|@count > 0}
			<div id="short_description_block">
				{if $product->description_short}
					<div id="short_description_content" class="rte align_justify"><h6>{$product->description_short}</h6></div>
				{/if}
			...
			<div class="prod-features large-12 columns">			
				<ul>
			...
			<div class="prod-buy row">
            	<div class="prod-price large-4 columns">
            		<!-- prices -->
            	...
            	<div class="prod-action row collapse">
					...
					<div id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) OR $virtual OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if} class="prod-quant large-3 columns">	
						<input type="text" name="qty" id="quantity_wanted" class="prod-quant" value="{if isset($quantityBackup)}{$quantityBackup|intval}{else}{if $product->minimal_quantity > 1}{$product->minimal_quantity}{else}1{/if}{/if}" size="2" maxlength="3" {if $product->minimal_quantity > 1}onkeyup="checkMinimalQuantity({$product->minimal_quantity});"{/if} />
	                </div>
	 		 ...
	 		 <!-- usefull links-->
			<ul id="usefull_link_block" class="large-12 columns">
			...
			{/if}
	         ...
	         <div class="web-dataproductos row">
				<!-- minimal quantity wanted -->
				<p id="minimal_quantity_wanted_p"{if $product->minimal_quantity <= 1 OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if} class="large-4 columns">
	            ...
	            <!-- availability -->
				<p id="availability_statut"{if ($product->quantity <= 0 && !$product->available_later && $allow_oosp) OR ($product->quantity > 0 && !$product->available_now) OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if} class="large-4 columns">
...
<!--	{if isset($product) && $product->description}
	...
	{if isset($accessories) AND $accessories}
		<!-- accessories -->
		<div id="accesories" class="web-modulo large-12 columns prod-rel">
    	<h4>{l s='Accessories'}</h4>
		
				<ul id="product_list" class="large-block-grid-3 prod-list">
							...
							<div class="prod-item ajax_block_product row collapse">
								...
								<div class="prod-buy">
		                            ...
		                            <div class="prod-price content_price large-4 columns">
		                            	 <span class="prod-amount">{if $priceDisplay != 1}{displayWtPrice p=$accessory.price}{else}{displayWtPrice p=$accessory.price_tax_exc}{/if}</span>
		                            </div>
									<div class="large-8 columns">							
							
										{if !$PS_CATALOG_MODE}
										
										<a class="exclusive button compra ajax_add_to_cart_button" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$accessory.id_product|intval}&amp;token={$static_token}&amp;add")}" rel="ajax_id_product_{$accessory.id_product|intval}" title="{l s='Add to cart'}">{l s='Add to cart'}</a>
										{/if}
